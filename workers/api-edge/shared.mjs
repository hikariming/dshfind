/**
 * API 边缘 Worker 的全部实现。worker.mjs 只是薄壳——workerd 把主模块的
 * 每个具名导出都当 entrypoint 校验，字符串常量导出会直接起不来，所以共享
 * 实现全部住在这里。
 *
 * 在 api.dshfind.com 上接管这几条读路径——
...
/** 与 Go publicDiscussionCacheControl 一致：用户刚写完就想看到，缓存窗口压到秒级。 */
export const CACHE_CONTROL_DISCUSSION = "public, max-age=15, s-maxage=30, stale-while-revalidate=300";
...
/** 复刻 handlePreflight：公开端点的 OPTIONS（CORS *）。 */
function publicPreflight(methods) {
...
/**
 * 论坛 + 鉴权的路由（S3）。方法与路径都要精确匹配，匹配不上的组合返回 null
 * 交给外层透传——Go 还活着时它的 mux 对 405 等有既定行为，没必要在边缘重造。
 */
async function routeForumAuth(request, env, url) {
  const { pathname } = url;
  const method = request.method;

  // ── 鉴权 ──
  if (pathname === "/auth/github" && method === "GET") return handleGitHubLogin(request, env, url);
  if (pathname === "/auth/github/callback" && method === "GET") {
    return handleGitHubCallback(request, env, url);
  }
  if (pathname === "/auth/me") {
    if (method === "GET") return handleAuthMe(request, env);
    if (method === "OPTIONS") return handleAuthMePreflight(request, env);
  }
  if (pathname === "/auth/logout" && method === "POST") return handleLogout(request, env, url);

  // ── BBS ──
  if (pathname === "/v1/forum/threads") {
    if (method === "GET" || method === "HEAD") return handleListThreads(request, env, url);
    if (method === "POST") return handleCreateThread(request, env);
    if (method === "OPTIONS") return credentialedPreflight(env, request, "GET, POST, OPTIONS");
  }
  let m = /^\/v1\/forum\/threads\/([^/]+)$/.exec(pathname);
  if (m) {
    const slug = decodeURIComponent(m[1]);
    if (method === "GET" || method === "HEAD") return handleThreadDetail(request, env, slug);
    if (method === "DELETE") return handleDeleteThread(request, env, slug);
    if (method === "OPTIONS") return credentialedPreflight(env, request, "GET, DELETE, OPTIONS");
  }
  m = /^\/v1\/forum\/threads\/([^/]+)\/posts$/.exec(pathname);
  if (m) {
    if (method === "POST") return handleThreadReply(request, env, decodeURIComponent(m[1]));
    if (method === "OPTIONS") return credentialedPreflight(env, request, "POST, OPTIONS");
  }
  m = /^\/v1\/forum\/posts\/([^/]+)$/.exec(pathname);
  if (m) {
    if (method === "DELETE") return handleDeletePost(request, env, decodeURIComponent(m[1]));
    if (method === "OPTIONS") return credentialedPreflight(env, request, "DELETE, OPTIONS");
  }

  // ── 插件讨论 / 投票 ──
  m = /^\/v1\/me\/plugin-votes\/([^/]+)\/([^/]+)$/.exec(pathname);
  if (m) {
    const [owner, repo] = [decodeURIComponent(m[1]), decodeURIComponent(m[2])];
    if (method === "GET") return handleMyPluginVote(request, env, owner, repo);
    if (method === "OPTIONS") return credentialedPreflight(env, request, "GET, OPTIONS");
  }
  m = /^\/v1\/plugins\/([^/]+)\/([^/]+)\/discussion$/.exec(pathname);
  if (m) {
    const [owner, repo] = [decodeURIComponent(m[1]), decodeURIComponent(m[2])];
    if (method === "GET" || method === "HEAD") return handleDiscussion(request, env, owner, repo);
    if (method === "OPTIONS") return publicPreflight("GET, OPTIONS");
  }
  m = /^\/v1\/plugins\/([^/]+)\/([^/]+)\/comments$/.exec(pathname);
  if (m) {
    const [owner, repo] = [decodeURIComponent(m[1]), decodeURIComponent(m[2])];
    if (method === "POST") return handlePluginComment(request, env, owner, repo);
    if (method === "OPTIONS") return credentialedPreflight(env, request, "POST, OPTIONS");
  }
  m = /^\/v1\/plugins\/([^/]+)\/([^/]+)\/vote$/.exec(pathname);
  if (m) {
    const [owner, repo] = [decodeURIComponent(m[1]), decodeURIComponent(m[2])];
    if (method === "PUT") return handlePluginVote(request, env, owner, repo);
    if (method === "DELETE") return handlePluginUnvote(request, env, owner, repo);
    if (method === "OPTIONS") return credentialedPreflight(env, request, "PUT, DELETE, OPTIONS");
  }
  return null;
}
...
export async function handleRequest(request, env) {
    ...
    // 论坛 + 鉴权（S3）：含写方法与 OPTIONS，必须在 readOnly 闸之前。
    // ⚠️ 这里 catch 不透传：写路径的数据在 D1，退回 Go 会写进已冻结的 Turso
    // 副本——静默写丢比一次 500 严重得多。
    try {
      const routed = await routeForumAuth(request, env, url);
      if (routed !== null) return routed;
    } catch (err) {
      console.error("forum/auth route failed", err);
    ...
    // 路由只把认识的路径送进来；其余一律透传。写方法（含 OPTIONS
