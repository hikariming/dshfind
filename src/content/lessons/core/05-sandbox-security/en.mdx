...
One level up is the policy layer of the filesystem stack (fs-policy plugin). It provides no service methods; it only participates through `fs/*` event gates, and is dedicated to "edit hygiene":
...
(Source: packages/fs/fs-policy/README.zh.md)
...
> Only restrictions that share the host's filesystem and kernel are supported. Containers, microVMs, and remote executors are not backends of this seam: they replace the Service provider of the entire capability seam (ctx.bash, ctx.fs) as an environment-consistent group.
>
> — Source: packages/sandbox/sandbox/README.zh.md
