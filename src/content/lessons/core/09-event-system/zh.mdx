...
|---|---|---|---|
| `agent/pre-step` | waterfall | 每个步骤开始前，带着本步要进入的消息批次 | 拒绝整步（reject），或替换/注入消息——plan-mode（计划模式）、workspace-context（工作区上下文）就在这里干活 |
...
| `fs/write-intent` | waterfall | 产生写文件意图时 | 安全策略：允许、拒绝或改写——fs-policy（文件策略）插件就在这里 |
