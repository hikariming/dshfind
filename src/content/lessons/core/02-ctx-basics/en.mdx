...
- Want to run commands, constrain the sandbox, or attach skills? The docs say `ctx.shell`, `ctx.sandbox`, and `ctx.skills`.
...
2. **Every capability is a service mounted on ctx**: `ctx.llm` models, `ctx.tools` tools, `ctx.sessions` sessions, `ctx.shell` commands, `ctx.sandbox` sandbox, `ctx.skills` skills… dozens of services, one entry point.
