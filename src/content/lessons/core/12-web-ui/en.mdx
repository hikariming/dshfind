...
|---|---|---|
| `apiproxy/` | The shared host API gateway and protocol conventions | `ctx.apiProxy` |
| `webserver/` | The HTTP routing transport | `ctx.webServer` |
| `frontend-static/` | The SPA dist server occupying webserver's fallback slot | consumes `ctx.webServer` |
| `directory-picker/` + `-native` / `-browse` / `-auto` | The workspace directory-picker seam and its three backends | `ctx.directoryPicker` |
| `plugin-inventory/` | A read-only projection of the current Loader entries | Remote `pluginInventory/list` |
