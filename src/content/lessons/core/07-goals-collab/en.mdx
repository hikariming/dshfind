...
- **Independent of consumers**: goal state and "how it is used" are separate. Its consumers can be a model-facing tool (`tool-goal`, letting the model read and update the goal), a user-facing command (`command-goal`, letting you view the goal on the command line or in the UI), and continuation policies (`goal-session`, responsible for "same-session goal continuation").
...
| `goal-session/` | Same-session goal continuation | none |
...
### 3.1 tasks: A Background Protocol for Long-Running Tools

`packages/tasks/README.zh.md` describes this family like this:
...
Family composition: `tasks/` defines the task registry and lifecycle conventions (`ctx.tasks`), `tasks-local/` provides the process-local registry implementation, and `tool-tasks/` exposes task control and completion notification to the model (registered on `ctx.tools`).
...
|---|---|---|
| `subagent-spawn/` | Starts a **brand-new** in-process sub-agent | A new colleague starting from scratch |
| `subagent-fork/` | Starts an in-process sub-agent from the parent agent's **completed history** | A colleague picking up the baton with the "backstory" |
...
Family composition: `workflow/` defines workflow execution and lifecycle events (`ctx.workflows`), `workflow-workerthread/` runs scripts in a thread, `tool-workflow/` exposes general workflow execution to the model, and `tool-ralph/` exposes the fixed Ralph workflow.
...
|---|---|---|---|
| Goal | Long tasks know where they are headed | `goal`, `goal-session`, `tool-goal`, `command-goal` | `ctx.goals` |
| Plan | Steps are auditable | `plan-mode` | `ctx.planMode` |
| Background tasks | Long-running operations don't block | `tasks`, `tasks-local`, `tool-tasks` | `ctx.tasks` |
...
| Workflow | Multi-agent orchestration | `workflow`, `workflow-workerthread`, `tool-workflow`, `tool-ralph` | `ctx.workflows` |
...
<Quiz
  ...
  questions={[
    ...
    },
    {
      ...
      answer: 1,
      explanation:
        "From the source: subagent-spawn \"starts a brand-new in-process sub-agent\" — the sub-agent starts from zero and knows nothing about the parent session; subagent-fork \"starts an in-process sub-agent from the parent agent's completed history\" — it inherits the parent session's history and suits continuation tasks. Out-of-process delegation (ACP, Codex, Claude Code, dsh-sdk) is a separate set of named providers.",
    },
    {
