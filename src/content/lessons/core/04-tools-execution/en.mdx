  ...
  questions={[
    ...
    },
    {
      ...
      options: [
        ...
        "It immediately returns a background job id; afterward, use job_output to read the output and job_kill to stop it",
      ...
      answer: 1,
      explanation:
        "Background mode applies no timeout, and the call immediately returns \"started background job\" plus the job id. The task is registered with the shared ctx.jobs background-job runtime, and ownership and cleanup are both recorded: job_output reads incremental output, job_list shows the task list, and job_kill stops tasks no longer needed — no unclaimed orphan processes.",
    },
    {
