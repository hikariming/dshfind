...
| 事件 | 模式 | 发生在哪一步 | 常见用途 |
|---|---|---|---|
| `agent/pre-step` | waterfall | 每个步骤开始前，带着本步要进入的消息批次 | 拒绝整步，或替换/注入消息——plan-mode（计划模式）、workspace-context（工作区上下文）就在这里 |
...
| `fs/write-intent` | waterfall | 产生写文件意图时 | 文件安全策略：fs-policy（文件策略插件）就在这里 |
