...
命令在后台继续跑。模型先去做别的事，之后用 `task_output` 读输出、用 `task_list` 看有哪些任务、用 `task_kill` 停掉不再需要的任务。后台任务在 DSH 里注册到通用的任务运行时 `ctx.tasks`，归属和清理都有记录——不会变成无人认领的「野进程」。
...
（来源：`docs/tool-catalog.zh.md` 的 bash 工具章节，schema 生成自 `packages/bash/tool-bash/src/index.ts`）
...
`ctx.bash` 是 bash 执行器 seam（接缝）的规范约定，模型侧的 `bash` 工具就注册在这条 seam 上：
...
- **后台**：`run_in_background: true`，立即返回 task id，由通用任务运行时 `ctx.tasks` 接管。
...
<Quiz
  ...
  questions={[
    ...
    },
    {
      ...
      options: [
        "命令会一直阻塞直到完成，和前台没有区别",
        "立即返回一个后台任务 id，之后用 task_output 读输出、用 task_kill 停止",
        "命令不会真正执行，只返回一个错误",
        "自动删除当前会话的工作目录",
      ],
      answer: 1,
      explanation:
        "后台模式不应用超时，调用立即返回 started background task 加任务 id。任务注册到通用的 ctx.tasks 运行时，归属和清理都有记录：task_output 读增量输出、task_list 看任务列表、task_kill 停止不再需要的任务——不会产生无人认领的野进程。",
    },
    {
      ...
      answer: 1,
      explanation:
        "packages/pty/README.zh.md 的原话：PTY 提供「持久且限定所有者范围的终端会话，适用于需要跨工具调用保留状态或使用交互式 stdin 的工作流」。它通过 terminal_open、terminal_send、terminal_read、terminal_signal、terminal_close、terminal_list 六个工具操作；每项操作都要求提供完全相同的发起 Agent——即使模型获知另一个 agent 的终端 id，也无法操作它（owner 隔离）。PTY 是单次 bash 的补充，不取代后者的逐操作约定。",
    },
    {
