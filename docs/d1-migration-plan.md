...
### 7.1.7 S3：论坛 + 投票 + 鉴权（2026-08-28 代码完成，2026-08-29 已切流）
...
**切流记录（2026-08-29T02:02Z）**：migrate 补增量 → push + deploy 验收通过
（验收含论坛列表与 auth/me 断言）→ `--verify` 三表行数一致 → 生产冒烟 12 项
全过（读路径归属、OAuth 302 带真实 client_id 与 3 个 state cookie、logout 清
cookie 属性逐项相同、未登录 401/坏 Origin 403/OPTIONS 204）→ **切流后 Go
audit 零新行**。Go 从此只剩 healthz 与 admin 端点（S4）。待办：真人过一遍
