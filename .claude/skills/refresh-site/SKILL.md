---
name: refresh-site
description: 跑一整轮站点数据刷新并上线：GitHub 同步（新插件/star）→ 头部插件下载量 → 头部插件安装方式 → 静态快照（首页三条 rail、插件库、排名）→ 构建验证 → 只提交生成物 → 推送触发 Cloudflare 部署。用于「好几天没更新了，把数据刷一遍」，也用于排查某一步为什么没生效。
---

# 站点数据刷新

一条命令：`pnpm refresh`（脚本 `scripts/refresh-site.mjs`）。五步串起来，任一步失败即中止，
工作区不会留下半截状态。

```sh
pnpm refresh                      # 同步 + 下载量 + 安装方式 + 重生成 + 构建 + 提交（不推）
...
首页的「编辑推荐 / 本周飙升 / 新面孔」全部来自第 4 步的 `home-picks.ts`——**不跑第 4 步，首页就不会换人**。
脚本跑完会打印这三条 rail 的进出名单，一眼能看出首页变了谁。

第 3 步是**头部全量重探**而不是按新鲜度增量：`npm_latest_version` 会作为精确 revision
发给 DSH 桌面端插件市场，桌面端照着它装。头部插件一周能发十几个版本，7 天的 stale
阈值根本追不上。脚本跑完会打印「桌面端市场契约变化」，谁开始能装、谁的版本变了一目了然。
全库补探（让新收录的仓库进得了市场）不在本流程里，见 desktop-market skill。
...
- 桌面端市场安装契约：`.claude/skills/desktop-market/SKILL.md`
